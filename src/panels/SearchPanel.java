...
public class SearchPanel extends JPanel {
	...
	private List<Patient> patientResults;
	private List<Visit> visitResults;
	...
	public SearchPanel() {
		...
		searchByPatients = database.getPatientSearchTypes().toArray();
		searchByVisits = database.getVisitSearchTypes().toArray();
	...
	*/
	private void showResults() {
		...
		if (index == 0) {
			results.setListData(patientResults);
		}
		else if (index == 1) {
			results.setListData(visitResults);
		}
	...
	*/
	private class SubmitListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (index == 0) {
				patientResults = database.findPatient(searchTerm.getText(), searchBy.getSelectedItem());
			}
			else if (index == 1) {
				visitResults = database.findVisit(searchTerm.getText(), searchBy.getSelectedItem());
			}
